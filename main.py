...
C_CURSOR    = np.array([255.0, 205.0, 50.0], dtype=np.float32)
C_CURSOR_U8 = np.array([255,   205,   50  ], dtype=np.uint8)
...
def extract_spectrum(audio_path: str, fps: int,
    ...
    norm   = np.clip((mel_db + 60.0) / 60.0, 0.0, 1.0)
    spec   = norm.T.astype(np.float32)
    return uniform_filter1d(spec, size=3, axis=0)
...
def generate(
    ...
    _xs        = np.arange(wf_w)
    _bar_idx   = np.minimum(_xs // bar_total, N_BARS - 1).astype(np.int32)
    _is_bar    = (_xs % bar_total) < max(1, bar_total - 1)
    _ys        = np.arange(wf_h, dtype=np.int32)[:, np.newaxis]
    ...
    def render_frame(fi: int) -> np.ndarray:
        ...
        amps_f    = frame_amps[spec_fi]
        bhs       = np.maximum(3, (amps_f * wf_h * 0.90).astype(int))
        by_starts = ((wf_h - bhs) // 2).astype(int)
        bhs_c     = bhs[_bar_idx]
        by_c      = by_starts[_bar_idx]
        bar_mask  = (_ys >= by_c) & (_ys < by_c + bhs_c) & _is_bar
        cur_bar   = cur_px // max(1, bar_total)
        played_m  = ((_bar_idx < cur_bar) & _is_bar)[np.newaxis, :]
